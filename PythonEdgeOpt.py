...
def append_result(entry, path="results.pkl"):
    """
...
def save_results(path: str = "results.pkl") -> None:
    """
...
def aggregate_models(client_datasets, node_models, G, tolerance, c, max_iters, rejection_threshold, K_decision, averaging, when, CorruptValue, true_nodes, Print_Val, noise_STD, PrivacyMethod, p, learning_rate, batch_size, input_size, hidden_size, num_classes, detect, log_filename, test_ds, criterion, save, CorruptClients, typeAttack, var, mean, Target, scale, num_clients, perm_threshold=0.5):
    ...
    while not converged and count < max_iters:
        ...
        variances = []
        for i in range(num_nodes):  # or use: for i in true_nodes
        ...
        # if count % 100 == 0:
        #     loss, acc = evaluate(node_models, test_ds, criterion, batch_size)
        ...
        if save == True:
            with open(log_filename, "a") as logfile:
                logfile.write(f"Round {count}  FAR={far:.3f} | MDR={mdr:.3f} | loss={loss:.4f} | acc={acc:.2f}%")
        count += 1
...
def main():
    ...
    K_decision = 8
    averaging = 1
    noise_levels = [0,0.01,0.1,1,10]  # Noise levels for Gaussian noise

    var = 10**9
    mean = 0
    Target = np.random.randint(1, num_clients)
    scale = 0
    typeAttackArray = [1] # 0:No attack , 1: Gaussian noise, 2: Copycat attack, 3: Gaussian addative noise attack, 4: LIE attack, 5 Sign flip attack, 6: Label Flipping attack
        ...
        for noise in noise_levels:
            ...
            for rnd in range(num_rounds):
                ...
                updated, err_hist, FAR_list, MDR_list, mask_history, loss_list, acc_list = aggregate_models(
                        ...
                        CorruptClients, typeAttack, var, mean, Target, num_clients, scale)
