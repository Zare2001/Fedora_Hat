...
def main():
    ...
    num_rounds = 50
    threshold = 0.02
    typeAttack = 6              # 0: No attack, 1: Gaussian noise, 2: Copycat attack, 3: Gaussian addative noise attack, 4: LIE attack, 5 Sign flip attack, 6: Label Flipping attack
    if typeAttack == 1:
    ...
    # PrimModulo = [2**32 - 5, 2**32 - 5, 2**61 - 1] # Prime number for SMPC should be above sum of all client local model values
     PrimModulo = [2**32 - 5, 2**32 - 5, 2**61 - 1] 
