...
def main():
    ...
    num_rounds = 50
    threshold = 0.02
    typeAttack = 3              # 0: No attack, 1: Gaussian noise, 2: Copycat attack, 3: Gaussian addative noise attack, 4: LIE attack, 5 Sign flip attack, 6: Label Flipping attack
    if typeAttack == 0:
    ...
    K_decision = 5
    averaging = 0.5
    noise_levels = [0,10**2,10**-2,10**0,10**1]  
    var = 10**3                 # Standard deviation for Gaussian noise
    mean = 0                   # Mean for Gaussian noise
    Target = np.random.randint(1, num_clients) # Target client for copycat attack
    scale = 0
    PrivacyMethod = 2       # 0: No privacy, 1: DP, 2: SMPC, 3: Subspace
