...
def train_local_model(dataset, global_model, num_epochs):
    """
    Fine-tunes a copy of `global_model` on `dataset` for `num_epochs`
    and returns:
        • the trained model
        • a list of epoch-average training losses
        • a list of epoch-average training accuracies
    """
...
def main():
    ...
    num_rounds = 400
    threshold = 0.02
    typeAttack = 0              # 0: No attack, 1: Gaussian noise, 2: Copycat attack, 3: Gaussian addative noise attack, 4: LIE attack, 5 Sign flip attack, 6: Label Flipping attack
    if typeAttack == 0:
    ...
    averaging = 1
    noise_levels = [0] 
    var = 10**8                 # Standard deviation for Gaussian noise
    ...
    for param in loop_params:
        ...
        for round in range(num_rounds):
            ...
            for client_idx in range(num_clients):
                ...
                local_models[client_idx],  epoch_losses[client_idx], epoch_accs[client_idx]= train_local_model(client_datasets[client_idx], local_models[client_idx], num_epochs)

            for client_idx in range(num_clients):
                sumLoss = sum(epoch_losses[client_idx])
                sumAcc = sum(epoch_accs[client_idx])
                avg_t_loss = sumLoss / len(epoch_losses[client_idx]) 
                avg_t_acc = sumAcc / len(epoch_accs[client_idx])
